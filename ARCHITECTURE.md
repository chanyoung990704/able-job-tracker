...
| `upsert_jobs.js` | Standalone Node script: pushes `data/jobs.json` into Supabase `jobs` + `job_strategies` (camelCase → snake_case, PostgREST `Prefer: resolution=merge-duplicates`, chunks of 100, `jobs` before `job_strategies` for the FK). Reads `SUPABASE_SERVICE_ROLE_KEY` from the environment or a gitignored `.env` — the anon key is read-only on these tables under RLS — and exits non-zero rather than silently skipping when it is absent. `--dry-run`, `--since=N`, and `--prune` to delete rows whose id has left `data/jobs.json` (refused alongside `--since`, and capped at 50 rows unless `--force-prune`, since a large prune usually means the local file is truncated). |
